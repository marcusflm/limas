<div>
    ...
    <x-card>
        ...
        <br>
        <div class="flex justify-end">
            <x-button label="{{ $pedido->status_pedido_id == 1 ? 'Fechar pedido' : 'Pedido fechado' }}" icon="{{ $pedido->status_pedido_id == 1 ? 'o-lock-open' : 'o-lock-closed' }}" wire:click="alterar_status_pedido({{ $pedido->id }})" class="btn + {{ $pedido->status_pedido_id == 1 ? 'btn-outline btn-primary' : 'bg-primary text-white'}}" />
        </div>
    </x-card>
    @else
